...
if (skew == "rgba(0, 89, 255, 0.25)") {
  ...
  document.getElementById("progress-container").style.backgroundColor =
    "rgba(0, 89, 255, 0.25)";
...
function analyse() {
  ...
  analysisInterval = setInterval(() => {
    ...
    document.getElementById("glow").style.boxShadow = `0px 0px ${
      generalVolume * 75
    }px ${generalVolume}px ${rgba}`;
  }, 0);
}
...
function uploadLink() {
  ...
  if (
    ...
    url.includes("https://music.youtube.com")
  ) {
    alert(
      "Downloading or streaming from YouTube is against YouTube Terms of Service"
    );
  } else {
...
function updateProgressClick(event) {
  ...
  const newTime =
    (percentage / 100) * document.getElementById("audio").duration;
  document.getElementById("audio").currentTime = newTime;
}

document
  .getElementById("progress-container")
  .addEventListener("click", updateProgressClick);
